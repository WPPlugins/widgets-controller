...
jQuery(document).ready(function() {
	...
	jQuery("select.list.pages option").live("click", function() {
		if(jQuery(this).hasClass("selected") == true) {
			jQuery(this).removeClass("selected").attr({"selected":false});
			jQuery(this).closest("div.widgets_controller_box").find("select.list.pages option[title='"+jQuery(this).attr('value')+"']").each(function() {
				jQuery(this).removeClass("selected").attr({"selected":false});
			});
		} else {
			jQuery(this).addClass("selected").attr({"selected":true});
			...
			jQuery(this).closest("div.widgets_controller_box").find("select.list.pages option[title='"+jQuery(this).attr('value')+"']").each(function() {
				jQuery(this).addClass("selected").attr({"selected":true});
				var sub_pagescopy = jQuery(this).clone();
				jQuery(this).closest("select").prepend(sub_pagescopy);
				jQuery(this).remove();
			});
		}
	...
	jQuery("input.checkbox.activepages").live("change", function() {
		...
		if(jQuery(this).attr("checked") == "checked") {
			...
			jQuery.post ( ajaxurl, {
				...
				jQuery(current_ele).closest("div.widgets_controller_box").find("select.list.pages option[title != 0]").each(function() {
					jQuery(this).css({"color":"red", "font-size":"11px"});
				});
